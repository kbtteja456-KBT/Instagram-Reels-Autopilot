...
export const CircuitStormCanvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Colors tuned for Instagram & Amber Cyber Aesthetics
    const ORANGE_COLORS = {
      core: '#ffffff',
      glow: '#fbbf24',
      trail: '#f59e0b',
    ...
    const spawnPulse = (nearMouse = false): CircuitPulse => {
      ...
      const color: 'orange' | 'green' | 'purple' = rand < 0.4 ? 'orange' : rand < 0.7 ? 'purple' : 'green';
    ...
    const render = (time: number) => {
      ...
      if (isCursorActive) {
        ...
        auraGrad.addColorStop(0.65, 'rgba(225, 48, 108, 0.15)');
      ...
      for (let i = 0; i < pulses.length; i++) {
        ...
        if (isCursorActive) {
          ...
          if (dist > 15 && dist < 700) {
            ...
            if (dist < 90 && Math.random() < 0.2) {
              ...
              const sparkColor = p.color === 'orange' ? ORANGE_COLORS.spark : p.color === 'purple' ? PURPLE_COLORS.spark : GREEN_COLORS.spark;
              const glowColor = p.color === 'orange' ? ORANGE_COLORS.glow : p.color === 'purple' ? PURPLE_COLORS.glow : GREEN_COLORS.glow;
        ...
        const pal = p.color === 'orange' ? ORANGE_COLORS : p.color === 'purple' ? PURPLE_COLORS : GREEN_COLORS;
