...
export const Header: React.FC<HeaderProps> = ({
  ...
  const displayName = channelTitle ? `@${channelTitle}` : (isLegacyOwner ? 'Creator (Instagram Pro)' : 'My Account');
  const initial = (channelTitle || 'I').charAt(0).toUpperCase();
  ...
  return (
    <header className="top-header">
      <div className="header-left">
        ...
        {isLegacyOwner ? (
          <div className="zero-cost-badge" style={{ borderColor: 'rgba(245, 158, 11, 0.4)', background: 'rgba(245, 158, 11, 0.1)' }}>
            ...
            <span style={{ color: '#f59e0b', fontWeight: 600 }}>Meta Graph API v19.0+</span>
          </div>
        ) : (
          <div
            onClick={onOpenVault}
          ...
          >
            ...
            <span>Trial: {trialVideosUsed}/{trialMaxVideos} Reels</span>
          </div>
        )}
      </div>
      ...
      <div className="header-right">
        ...
        <div className="user-profile-pill" title="Connected Instagram Account">
          {channelAvatar ? (
        ...
        >
          <span>{isGenerating ? 'Queuing Generation...' : '+ Create Reel Now'}</span>
        </button>
      </div>
    </header>
  );
